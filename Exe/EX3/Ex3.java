...
public class Ex3 {
	...
	//////////////////////////////////////ADDED
	private static Point2D firstPoint = null; // for saving the prev point if needed
	...
	public static void mouseClicked(Point2D p) {
		...
		if(_mode.equals("Rect")) {
			if (firstPoint==null) {  					// check if the current click is the first or second
				firstPoint = p;       					//save the first point in the variable firstPoint
			}
			else {                    					// second click
				_map.drawRect(firstPoint, p, col);	   	// drow the rect
				firstPoint = null;                      // reset the firsPoint variable in order to save the next point clicked
				_mode = "" ;							// reset the mode in order to to block the mode 
			}
		}
		...
		if(_mode.equals("Segment")) {
			if (firstPoint==null) {  					// check if the current click is the first or second
			...
			else if (p.ix() == firstPoint.ix() && p.iy() == firstPoint.iy()) {
				...
				_map.drawSegment(firstPoint, p, col);	   	// drow the rect
				firstPoint = null;                      // reset the firsPoint variable in order to save the next point clicked
				_mode = "" ;							// reset the mode in order to to block the mode 
			}
		}
		...
		if(_mode.equals("Circle")) {
			...
			if (firstPoint==null) {  					// check if the current click is the first or second
				...
				double rad = firstPoint.distance(p);
				_map.drawCircle(firstPoint, rad, col);	   	// drow the rect
				firstPoint = null;                      // reset the firsPoint variable in order to save the next point clicked
				_mode = "" ;							// reset the mode in order to to block the mode 
			}
		...
		if(_mode.equals("ShortestPath")) {
			if (firstPoint==null) {  					// check if the current click is the first or second
				...
				_map.shortestPathDist(firstPoint, p);	   	// drow the rect
				Point2D [] ans=_map.shortestPath(firstPoint, p);///ADdED //ADDED !!!
